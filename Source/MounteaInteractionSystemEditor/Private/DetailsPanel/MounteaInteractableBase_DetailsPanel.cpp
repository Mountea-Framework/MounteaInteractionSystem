...
#define LOCTEXT_NAMESPACE "MounteaInteractableComponentsPanel"
...
void MounteaInteractableBase_DetailsPanel::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	const TWeakObjectPtr<UMounteaInteractableComponentBase> weakComponent = Cast<UMounteaInteractableComponentBase>(ObjectsBeingCustomized[0].Get());
	...
	ItrCategoryBuild.AddCustomRow(LOCTEXT("MounteaInteractableComponentsPanel_Defaults", "Load Defaults"), false)
	.WholeRowWidget
	[
		SNew(SBox)
		.HAlign(HAlign_Fill)
		[
			...
			[
				...
				.HAlign(HAlign_Fill)
				.Text(LOCTEXT("MounteaInteractableComponentsPanel_Defaults_Text", "Load Defaults"))
				.ToolTipText(LOCTEXT("MounteaInteractableComponentsPanel_Defaults_Tooltip", "Overrides data with default values from Project Settings.\nProject Settings must be defined!"))
